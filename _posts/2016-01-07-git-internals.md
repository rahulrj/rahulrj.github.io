...
**- 3 Way Merge-** But suppose we want to merge `test-branch` into `test-branch2`. Now there is no direct path between C3 and C4 that we can follow. So here git follows `3-way merge` using the nearest common ancestor(C2) of two branches. Here the common ancestor is also required in the merge process because suppose there are changes at the same line in both branches (C3 and C4). Without consulting the base-branch and checking the contents at the same line in the base branch, git wont be able to automatically resolve the conflict.Also, a merge commit will always have two parents.
